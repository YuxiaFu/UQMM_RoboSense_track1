...
def convert_img_path(dataset_root, img_path):
    abs_path = os.path.join(dataset_root, img_path[len("data/nuscenes/"):])
    return abs_path
...
def main(root, dataset_root, file_path, output_file):
    ...
    with open(os.path.join(root, file_path), 'r', encoding='utf-8') as f:
        ...
        for sample in data:
            ...
            new_sample['image'] = os.path.join(dataset_root, convert_img_path(dataset_root, sample['image_path']['CAM_FRONT']))
            new_sample['images'] = [os.path.join(dataset_root, convert_img_path(dataset_root, sample['image_path'][cam])) for cam in CAM_NAME]
...
if __name__ == "__main__":
    ...
    file_path = 'drivebench-test.json'
    output_file = 'drivebench_test_converted2senna.json'
